...
"""
Script:	accounts-users.py
Date:	2020-01-10
Platform: macOS/Linux
Description:
Retrieves and processes the users object of /JSSResource/accounts/
"""
__author__ = 'thedzy'
...
def get(api_classic=None, api_universal=None, repo_path=None):
    ...
    if api_query.success:
        data_objects = api_query.data['accounts']['users']
        ...
        for data_object in data_objects:
            object_query = api_classic.get_data('accounts', 'userid', data_object['id'])
...
def clean_data(json_data):
    ...
    # Privileges appear in random order, requires sorting
    for privilege in json_data['account']['privileges']:
        json_data['account']['privileges'][privilege].sort()
