...
class JamfClassic:
    ...
    def __init__(self, api_url, usernmae, password, *args, **kwargs):
        """
        Initialisation method
        :param api_url: (str) url of the api
        :param usernmae: (str) username
        :param password: (str) password
        :param args: (list)
        :param kwargs: (dict)
        """
        self._api_url = api_url
        self._usernmae = usernmae
    ...
    def get_data(self, *objects, **kwargs):
        ...
        try:
            request = requests.get(request_url, auth=(self._usernmae, self._password),
    ...
    def del_data(self, *objects, **kwargs):
        ...
        request_url = '{0}/JSSResource/{1}'.format(self._api_url, '/'.join(str(arg) for arg in objects))
        try:
            request = requests.delete(request_url, auth=(self._usernmae, self._password),
                                      headers=self._headers, timeout=self._timeout, verify=self._verify)
        except requests.exceptions.HTTPError as err:
            return APIResponse(url=request_url, err=err)
    ...
    def put_data(self, data, *objects, **kwargs):
        ...
        request_url = '{0}/JSSResource/{1}'.format(self._api_url, '/'.join(str(arg) for arg in objects))
        try:
            request = requests.put(request_url, auth=(self._usernmae, self._password),
    ...
    def post_data(self, data, *objects, **kwargs):
        ...
        request_url = '{0}/JSSResource/{1}'.format(self._api_url, '/'.join(str(arg) for arg in objects))
        try:
            request = requests.post(request_url, auth=(self._usernmae, self._password),
                                    headers=self._headers, timeout=self._timeout, verify=self._verify, data=data)
        except requests.exceptions.HTTPError as err:
            return APIResponse(url=request_url, err=err)
